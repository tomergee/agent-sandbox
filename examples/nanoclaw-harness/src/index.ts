...
async function main(): Promise<void> {
  ...
  // 2. Container runtime
  if (process.env.RUNNER_TYPE !== 'agent-sandbox') {
  ...
  if (process.env.RUNNER_TYPE === 'agent-sandbox') {
    const { startSandboxCallbackServer } = await import('./sandbox-callback-server.js');
    startSandboxCallbackServer();
  }
...
async function shutdown(signal: string): Promise<void> {
  ...
  stopHostSweep();
  if (process.env.RUNNER_TYPE === 'agent-sandbox') {
    const { stopSandboxCallbackServer } = await import('./sandbox-callback-server.js');
    await stopSandboxCallbackServer();
  }
  try {
