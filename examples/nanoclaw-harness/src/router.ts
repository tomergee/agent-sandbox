...
import { wakeContainer, getContainerInfo } from './container-runner.js';
  ...
  if (runnerType === 'agent-sandbox') {
    if (wake) {
      ...
      if (freshSession) {
        ...
        const info = getContainerInfo(session.id);
        if (info && info.podIP) {
          ...
          try {
            const response = await fetch(`http://${info.podIP}:3000/message`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(messagePayload),
            });
            if (!response.ok) {
              log.error('HTTP request to sandbox failed', { status: response.status });
              stopTypingRefresh(freshSession.id);
            } else {
              log.info('Message sent to Agent Sandbox via HTTP');
            }
          } catch (err) {
            log.error('Failed to send message to sandbox via HTTP', { err });
            stopTypingRefresh(freshSession.id);
          }
        } else {
          log.error('Pod IP missing after wake', { sessionId: session.id });
          stopTypingRefresh(freshSession.id);
        }
      }
    } else {
      ...
      if (info && info.podIP) {
        ...
        fetch(`http://${info.podIP}:3000/message`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(messagePayload),
        }).catch(err => log.error('Failed to send accumulate message', { err }));
      }
    }
  } else {
