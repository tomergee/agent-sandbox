...
import { AgentSandboxRunner } from './agent-sandbox-runner.js';
...
const activeContainers = new Map<string, { process?: ChildProcess; containerName: string; runnerType?: string; podIP?: string }>();
...
export function getContainerInfo(sessionId: string) {
 ...
 */
async function spawnAgentSandbox(session: Session): Promise<void> {
  ...
  const runner = new AgentSandboxRunner();
  const claimName = `nanoclaw-${session.id}`;
  const namespace = process.env.AGENT_SANDBOX_NAMESPACE || 'default';
  const template = process.env.AGENT_SANDBOX_TEMPLATE || 'python-sandbox-template';

  log.info('Spawning Agent Sandbox', { sessionId: session.id, agentGroup: agentGroup.name, claimName });

  try {
    await runner.createSandboxClaim({
      name: claimName,
      template: template,
      namespace: namespace,
      env: [
        { name: 'PROVIDER', value: 'claude' },
        { name: 'ASSISTANT_NAME', value: agentGroup.name },
        { name: 'GROUP_NAME', value: agentGroup.name },
        { name: 'AGENT_GROUP_ID', value: agentGroup.id },
        { name: 'RUNNER_TYPE', value: 'agent-sandbox' },
        { name: 'HOST_CALLBACK_URL', value: process.env.HOST_CALLBACK_URL || 'http://nanoclaw-host-svc:3001/callback' },
        { name: 'SESSION_ID', value: session.id },
      ],
    });

    const sandboxName = await runner.resolveSandboxName(claimName, namespace, 60);
    const podIP = await runner.waitForSandboxReady(sandboxName, namespace, 180);

    log.info('Agent Sandbox ready', { claimName, podIP });

    activeContainers.set(session.id, { 
      containerName: claimName, 
      runnerType: 'agent-sandbox',
      podIP: podIP 
    });
    
    markContainerRunning(session.id);
  } catch (err) {
    log.error('Failed to spawn Agent Sandbox', { sessionId: session.id, err });
    throw err;
  }
}
...
export function wakeContainer(session: Session): Promise<boolean> {
  ...
  const runnerType = process.env.RUNNER_TYPE || 'docker';
  const promise = (runnerType === 'agent-sandbox' ? spawnAgentSandbox(session) : spawnContainer(session))
    .then(() => true)
    .catch((err) => {
...
export function killContainer(sessionId: string, reason: string): void {
  ...
  try {
    if (entry.runnerType === 'agent-sandbox') {
      const runner = new AgentSandboxRunner();
      const namespace = process.env.AGENT_SANDBOX_NAMESPACE || 'default';
      runner.deleteSandboxClaim(entry.containerName, namespace).catch(err => {
        log.error('Failed to delete SandboxClaim on kill', { sessionId, err });
      });
    } else {
