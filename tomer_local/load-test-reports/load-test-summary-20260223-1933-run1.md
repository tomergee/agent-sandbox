# Agent Sandbox Load Test Summary (Run 1) - Sandbox Only

## Cluster Configuration
- **Cluster Name**: `agent-sandbox-load`
- **Provider**: Google Kubernetes Engine (GKE)
- **Node Configuration**: 3 Nodes of `e2-standard-8` (which provides 8 vCPUs and 32 GB memory per node). This gave a total cluster capacity of 24 vCPUs and 96 GB memory.
- **Zone**: `us-central1-c`
...
- **Total Execution Time**: 61.07 seconds
...
| 02 | Create Sandboxes | 2.05s |
| 03 | Wait for Sandboxes ready | 5.31s |
| 04 | Gather Results | 0.37s |
| 05 | Delete Sandboxes | 2.07s |
...
The `agent-sandbox` controller performed efficiently, specifically spinning up the requested sandboxes and reaching a Ready state very quickly (~5.31s total wait time).
