...
QPS=${1:-50}
REPLICAS=${2:-50}
WARM_POOL_REPLICAS=${3:-20}
RUN_ID=$(date +%Y%m%d-%H%M%S)

# Base directories
DEV_DIR="/usr/local/google/home/glottman/dev/jetski_main"
AGENTS_DIR="${DEV_DIR}/branches/sandboxclaim-perf-improvements"
...
cat <<EOF > "${AGENTS_DIR}/tomer_local/scripts/testoverrides.json"
{
  ...
  "CL2_WARM_POOL_REPLICAS": $WARM_POOL_REPLICAS,
  "CL2_NAMESPACE_PREFIX": "agent-sandbox"
}
EOF

# 2. Port-forward metrics in background (if not already running)
pkill -f "kubectl port-forward -n agent-sandbox-system deployment/agent-sandbox-controller 8080:8080" 2>/dev/null || true
kubectl port-forward -n agent-sandbox-system deployment/agent-sandbox-controller 8080:8080 > /dev/null 2>&1 &
...
# 3b. Background log capture
echo "Starting background log capture..."
CONTROLLER_POD=$(kubectl get pods -n agent-sandbox-system -l app=agent-sandbox-controller -o jsonpath='{.items[0].metadata.name}')
(
  kubectl logs -f $CONTROLLER_POD -n agent-sandbox-system > "${LOGS_DIR}/controller-logs-live-${RUN_ID}.log"
) &
LOG_CAPTURE_PID=$!
...
echo "Stopping background log capture..."
kill $LOG_CAPTURE_PID 2>/dev/null
